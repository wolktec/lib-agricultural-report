...
export const getTotalHourmeter = (hourmeters: Telemetry[]): number => {
    ...
    if (hourmeterWithoutAnomalies.length > 0) {
        const firstHourmeter = Number(hourmeterWithoutAnomalies[0]);

        let lastHourmeter =
            hourmeterWithoutAnomalies.length > 1
                ? Number(hourmeterWithoutAnomalies[hourmeterWithoutAnomalies.length - 1])
                : firstHourmeter;
        if (lastHourmeter <= firstHourmeter && hourmeterWithoutAnomalies.length > 1) {
